...
public class EmailSender
{
    ...
    {
        logger.LogInformation("Sending order confirmation email to {Email}", order.CustomerDetails.Email);
        ...
        {
            body.AppendLine($"<li>{line.EventName} ({line.ArtistName}) ${line.Price} Number of tickets:{line.TicketCount}</li>");
        }
        body.AppendLine("</ul>");
        ...
        await daprClient.InvokeBindingAsync("sendmail", "create", body.ToString(), metadata);
    }
}
